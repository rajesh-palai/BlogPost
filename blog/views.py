...
def post_list(request):
    ...
    context = {
        'posts': posts,
        'page_range': page_range, #proper pagination way
    }
...
def post_detail(request, id, slug):
    ...
    if request.is_ajax():
        html = render_to_string("blog/comments.html", context, request)
...
def like_post(request):
    ...
    #return HttpResponseRedirect(post.get_absolute_url())
    if request.is_ajax():
        html= render_to_string("blog/like_section.html",context ,request)
        return JsonResponse({'form': html})
...
def favourite_post_list(request):
    fav_posts= request.user.favourite.all()
    context= {
        "fav_posts": fav_posts,
    }
...
def post_edit(request, id):
    ...
    if request.method=="POST":
        ...
        if form.is_valid() and formset.is_valid():
            ...
            for index, f in enumerate(formset):
                if f.cleaned_data:
                    if f.cleaned_data['id'] is None:
                        photo = Images(post=post, image = f.cleaned_data['image'])
                        photo.save()
                    elif f.cleaned_data['image'] is False:    #to delete the photo as using clear button photo will be false but id will exist
            ...
            return HttpResponseRedirect(post.get_absolute_url())  #to get post detail view
    else:
